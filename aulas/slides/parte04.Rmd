...
## Segmentos, estrelas e outras representações
  Dados Multivariados podem ser representados por um glifo ou ícone de duas dimensões. Algumas representações com ícones podem ser melhor apresentadas em uma tabela. Entretanto, não é muito prático quando se tem uma quantidade muito grande dados. A seguir mostra-se exemplos usando star plots e segment plot. Esse tipo de gráfico é fácilmente obtido no r usando a função stars(graphics),
...
a <- x$CW * x$CL
x[4:8] <- x[4:8] / sqrt(a)
palette(gray(seq(.4, .95, len = 5))) 
stars(x[4:8], draw.segments = TRUE,
labels = as.character(x$sp), nrow = 4,
...
## Exercício 1
Gere 200 observações aleatórias da distribuição normal multivariada com o vetor de média sendo $\mu = (0,1,2)$ e a matriz de covariância:

$$\sum =\left[\begin{array}{rrr}
1.0&-0.5&0.5\\
-0.5&1.0&-0.5\\
0.5&-0.5&1.0\\
...
Construa o Scatterplot matrix e verifique a localização e correlação para cada gráfico com os parâmetros da normal bivariada.

## Exercício 2
Sejam duas variáveis aleatórias $X$ e $Y$ independentes e identicamente distribuidas com mistura de normais. As componentes da mistura tem $N(0,1)$ e $N(3,1$) e com probabilidades $p_{1}$ e $p_{2}= 1 - p_{1}$ respectivamente. Gere uma distribuição normal bivariada com a distribuição $(X,Y)$ e construa um gráfico de contorno com o contourplot. 
